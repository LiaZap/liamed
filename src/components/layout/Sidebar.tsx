...
export function Sidebar({
  ...
  const filteredItems = menuItems.filter((item) => {
    if (item.hideFromGestor && user?.role === "GESTOR") {
      return false;
    }
    if (item.gestorOnly) {
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-full bg-white dark:bg-[#222428] border-r border-slate-200 dark:border-slate-800 w-[240px] transition-colors",
        className,
      )}
    >
      {/* Logo Section */}
